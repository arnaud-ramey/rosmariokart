...
public:
  ...
  bool init() {
    ...
    _nh_private.param("number_of_cols", _max_col, 2);
    ...
    while (true) {
      ...
      _nh_private.param( text, p._name, std::string(""));
      if (p._name.empty())
          break;
    ...
    int n_col= std::min(_max_col, (int) _nplayers);
    ...
    int id = 0; // players ind.

    for (int n_row = 0; n_row < n_line ;n_row++){
        int j = 0;
        while ( (id <= (int)_nplayers) && ( j <n_col) ){
          _players[id]._tl_win   = Point2d( j*player_w, n_row*player_h );
          _players[id]._item_tl_corner = Point2d ( (j+1)*player_w - _item_w*1.1,
                                                  n_row*player_h + 10 );
          j++;id++;
        }
    }
    ...
    for (unsigned int i = 0; i < _nplayers; ++i) {
      ...
      else if (p->_name.find("stage") != std::string::npos)
  ...
  bool render_countdown2texture(const int time, int r, int g, int b) {
    ...
    return _countdown_texture.loadFromRenderedText(renderer, _countdown_font,
                                                   time_str.str(), r, g, b);
  }
